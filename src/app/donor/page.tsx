...
export default function DonorPage() {
    ...
    (async () => {
      try {
        ...
        const { data: notifs } = await supabase
          .from('notifications')
          .select('id, conversation_id, donation_id')
          .eq('user_id', uid)
          .eq('read', false);
        ...
        notifs.forEach((n: any) => {
          if (n?.donation_id) map[String(n.donation_id)] = true;
          else if (n?.conversation_id) convIds.add(String(n.conversation_id));
        });
        if (convIds.size > 0) {
  ...
  useEffect(() => {
    // realtime notifications subscription for donor
        ...
        channel = supabase
          ...
          .on(
            ...
            async (payload: any) => {
              ...
              try {
                if (n?.donation_id) {
                  setUnread(prev => ({ ...prev, [String(n.donation_id)]: true }));
                  return;
                }
                ...
                const { data: conv } = await supabase
                  ...
                  .limit(1)
                  .maybeSingle();
                if (conv?.donation_id) setUnread(prev => ({ ...prev, [String(conv.donation_id)]: true }));
              } catch {}
            }
          )
          .subscribe();
        notifRef.current = channel;
      } catch {}
    })();

    return () => {
      if (notifRef.current) {
        try { supabase.removeChannel(notifRef.current); } catch {}
  ...
  const openChat = async (donationId: string) => {
    ...
    try {
      ...
      if (existing?.id) {
        // clear DB notifications for this conversation + current user, and clear local unread
        try {
          await supabase.from('notifications').delete().eq('conversation_id', existing.id).eq('user_id', userData.user.id);
        } catch {}
        setUnread(prev => { const copy = { ...prev }; delete copy[donationId]; return copy; });
      ...
      // new conversation - clear local unread and navigate
      setUnread(prev => { const copy = { ...prev }; delete copy[donationId]; return copy; });
    ...
    <div className="container py-8">
      <div className="flex flex-col items-start gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <Button asChild size="lg">
          <Link href="/donor/donate" aria-label="Donate Food" className="flex items-center">
              ...
              {donation.taken && (
                <div className="mt-4 flex flex-col gap-2">
                  <div className="relative inline-block w-full">
                    ...
                    {unread[String(donation.id)] && (
                      <span className="absolute -top-1 -right-1 h-3 w-3 bg-red-600 rounded-full ring-2 ring-white" aria-hidden />
                    )}
                  </div>
                  <Button variant="secondary" className="w-full" aria-label={`Mark donation ${donation.id} as complete`}>Complete</Button>
                </div>
              )}
            </div>
          ))}
