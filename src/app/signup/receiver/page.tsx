...
export default function ReceiverSignup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (data.user) {
      await supabase.from("profiles").insert([{ id: data.user.id, role: "receiver" }]);
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <form onSubmit={handleSignup} className="bg-white p-6 rounded-2xl shadow-md w-96">
        ...
        <button type="submit" className="w-full bg-green-500 text-white p-2 rounded-xl hover:bg-green-600">
          Signup
        </button>
        {message && (
