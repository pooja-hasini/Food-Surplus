...
async function geocodeAddress(address: string): Promise<{ lat: number; lng: number } | null> {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    // Build the row object and attempt to insert. If the DB schema does not have latitude/longitude
    // columns (some deployments), retry without those fields so submission still succeeds.
    ...
    try {
      const res = await supabase.from('food_listings').insert([row]).select();
    ...
    // If insert failed due to missing latitude/longitude columns, retry without them
    if (insertError && /latitude|longitude|column/i.test(String(insertError.message || insertError))) {
      // remove coords and retry
      delete row.latitude;
      delete row.longitude;
      try {
        const res2 = await supabase.from('food_listings').insert([row]).select();
        insertedRows = res2.data;
        insertError = res2.error;
        toast({ title: 'Note', description: 'Saved donation without numeric coordinates because the database lacks latitude/longitude columns.' });
      } catch (err2: any) {
        insertError = err2;
      }
    }
            ...
            <Textarea
              ...
              {...form.register('location', {
                onBlur: async (e) => {
                  ...
                  if (val && val.length > 5 && !/^\s*-?\d+(\.\d+)?\s*,\s*-?\d+(\.\d+)?\s*$/.test(val)) {
                    setGeoLoading(true);
                    setGeoError(null);
                    ...
                    if (result) {
                      setCoords(result);
                      toast({ title: 'Address resolved', description: `Found coordinates: ${result.lat.toFixed(5)}, ${result.lng.toFixed(5)}` });
                    } else {
                      setGeoError('Could not resolve address to coordinates.');
                    }
                  }
                },
              })}
            ...
            <div className="flex items-center gap-2 mt-2">
              <Button type="button" onClick={async () => {
                if (!navigator.geolocation) {
                  toast({ title: 'Geolocation unsupported', description: 'Your browser does not support geolocation.' });
                  return;
                }
                setLocLoading(true);
                navigator.geolocation.getCurrentPosition((p) => {
                  ...
                  setCoords({ lat, lng });
                  setLocLoading(false);
                  toast({ title: 'Location captured', description: 'Using your current location for this donation.' });
                }, (err) => {
                  setLocLoading(false);
                  toast({ title: 'Location error', description: err?.message || 'Unable to get location.' });
                }, { enableHighAccuracy: true, timeout: 10000 });
              }} disabled={locLoading}>
                {locLoading ? 'Capturing…' : 'Use my current location'}
              </Button>
              {coords && (<div className="text-sm text-muted-foreground">Using: {coords.lat.toFixed(5)}, {coords.lng.toFixed(5)}</div>)}
              {geoLoading && (<div className="text-sm text-muted-foreground">Resolving address…</div>)}
